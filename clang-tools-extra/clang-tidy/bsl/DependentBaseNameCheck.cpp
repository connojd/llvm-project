  ...
  std::vector<const NamedDecl *> &BaseNames)
{
  ...
  bool allPrivateNames = true;

  for (auto i = 0U; i < BaseNames.size(); i++) {
    if (BaseNames[i]->getAccess() == AS_public ||
        BaseNames[i]->getAccess() == AS_protected) {
      allPrivateNames = false;
    }
  }

  return !BaseNames.empty() && !allPrivateNames;
}

void DependentBaseNameCheck::printBaseNames(
  const std::vector<const NamedDecl *> &BaseNames,
  const StringRef &Name)
{
  for (size_t i = 0U; i < BaseNames.size(); i++) {
    const NamedDecl *BaseName = BaseNames[i];

    if (BaseName->getAccess() != AS_public &&
        BaseName->getAccess() != AS_protected) {
      continue;
    }
    ...
    if (i == 0) {
      ...
      diag(BaseName->getLocation(),
        "and found declaration of '%0' in base class here",
        DiagnosticIDs::Note) << Name;
    }
  }
}
  ...
  const DeclRefExpr *DeclRef)
{
  ...
  diag(DeclRef->getExprLoc(),
       "multiple declarations with name %0 found; need to qualify the name "
       "here") << Name;
  diag(VD->getLocation(), "using the declaration of %0 "
       "found here", DiagnosticIDs::Note) << Name;

  printBaseNames(BaseNames, VD->getName());
}
